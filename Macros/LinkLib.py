...
"""
    ...
    ╚═══════════════════════════════════════════════╝
"""
def makeExpressionPart( attPart, attLCS, linkName, linkLCS ):
	...
	if linkName and PyFeaturePrefix:
		PyFeatureName = PyFeaturePrefix + linkName
	# if everything is defined
	if attPart and attLCS and PyFeatureName and linkLCS:
		...
		if attPart == 'Parent Assembly':
			expr = '<<'+attLCS+'>>.Placement.multiply( <<'+PyFeatureName+'>>.Offset ).multiply( .<<'+linkLCS+'.>>.Placement.inverse() )'
		else:
			expr = '<<'+attPart+'>>.Placement.multiply( <<'+attPart+'>>.<<'+attLCS+'.>>.Placement ).multiply( <<'+PyFeatureName+'>>.Offset ).multiply( .<<'+linkLCS+'.>>.Placement.inverse() )'			
	else:
		expr = False
	return expr
